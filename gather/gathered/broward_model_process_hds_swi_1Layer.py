...
#--load the bottom of the biscayne for masking
bis_bot = np.loadtxt('ref\\Q1_bot.ref')
...
for i,dt in enumerate(bro.sp_start):
    if i%sp_step == 0:
        ...
        z1,totim,success = zetaObj.get_array(ipos)               

        z1 = z1[0,:,:]        
        z1m = np.ma.masked_where(z1 < bis_bot,z1)
        ...
        p3 = ax3.imshow(z1m,extent=imshow_extent,interpolation='none',vmax=zelev_max,vmin=zelev_min)
        p4 = ax4.imshow(dz1,extent=imshow_extent,interpolation='none')        
