...
for sn,ib in zip(nwis_sitenos,nwis_icbnd):
...
clusters = {}
for i1,[pt1,site1,ib1] in enumerate(zip(nwis_pts,nwis_sitenos,nwis_icbnd)):
    if ib1 > 0:
        print 'searching point ',i1
 
        for i2,[pt2,site2,ib2] in enumerate(zip(nwis_pts[i1+1:],nwis_sitenos[i1+1:],nwis_icbnd[i1+1:])):
...
smp.records = df
smp.save('relconc_thinned.smp',dropna=True)
