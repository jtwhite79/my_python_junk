...
#--lower left x,y and rotation counter clockwise
offset = [399072.272,1408238.74,15.0]
...
for c in range(0,ncol):
    ...
    for r in range(0,nrow):
        ...
        if offset[2] != 0.0:
            [centroid] = rotate([centroid],offset[2])
        
        #--add the offset in after the rotation
        [centroid] = add_offset([centroid],offset)
        
        #print nrow-r,c+1

        wr.poly(parts=[[centroid]], shapeType=1)
        wr.record([nrow-r,c+1,delr[r],delc[c],cell_count])
        cell_count += 1
        #break
    #break
        
wr.save(target='model_centroid')
