...
def get_xsec_points(names,xsec_attri,branch_points,branch_lengths,chain_breaks):
    branch_points_break = []
    branch_lengths_break = []
...
#--get_xsection points
xsec_points = get_xsec_points(names,xsec_attri,\
             branch_points,branch_lengths,chain_breaks)
...
xsec_points = m_2_ft(xsec_points)
...
wr.field('chainage',fieldType='N',size=50)

for x_at,x_pt in zip(xsec_attri,xsec_points):
    if x_pt != None:    
        wr.poly(parts=[[x_pt]], shapeType=1)
        wr.record([x_at[0],x_at[1],x_at[2]])
    
    else:
        print 'No point found for :',x_at[0],x_at[1],x_at[2]
       
    
    #if i > 2 :
    #    break       
    
wr.save(target='..\\shapes\\she_xsec_points')
