...
for mon in range(1,13):
    this_month_data = this_data[np.where(this_data[:,6]==mon)]
    this_precip = this_month_data[:,3]
    #this_precip = np.delete(this_precip,np.where(this_precip>=mask_val)) 
    #this_precip = ma.masked_where(this_precip<0.002,this_precip)
    this_hist,this_binedges = np.histogram(this_precip,bins=bins,range=(0.002,mask_val))
    print mon,max(this_hist),np.max(this_binedges)
    ax = pylab.subplot(13,1,mon)
    ax.bar(this_binedges[:-1],np.log10(this_hist),width=0.1)
    ax.set_xlim(0.002,mask_val)
    ax.set_title(str(mon))
    
this_precip = this_data[:,3]

this_hist,this_binedges = np.histogram(this_precip,bins=bins,range=(0.002,mask_val))
print mon,max(this_hist),np.max(this_binedges)
ax = pylab.subplot(13,1,13)
ax.bar(this_binedges[:-1],np.log10(this_hist),width=0.1)
ax.set_xlim(0.002,mask_val)
...
print np.shape(this_data),np.shape(this_t),np.shape(this_precip)
ax = pylab.subplot(1,1,1)
ax.plot(this_t,this_precip,label=str(83163))
#ax.legend()
ax.set_title(str(83163))
...
ax.text(698500,5,'Coopid #: '+str(83163))
