...
ax2 = pylab.twinx()
ax2.plot(np.arange(1900,1900+len(pdo_yr)),pdo_yr,'k--',lw=2.0)
...
for mon in range(6,12):

                                                                                                       
    
    ax = pylab.subplot(3,2,mon+1-6)                                                                            
    print np.shape(trans_draws)
    ...
    ax.set_ylim(0.0,0.8)
    if mon < 7: ax.set_xticklabels([])
    

pylab.figtext(0.5,0.95,'Monthly Distribution of Transition Probabilities and Annual PDO Index',ha='center')
