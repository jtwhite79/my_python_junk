...
xmax,ymax = 979135.0,782769.0

delta = 500.0
nrow = int(np.ceil((ymax-offset[1])/delta))
ncol = int(np.ceil((xmax-offset[0])/delta))
...
for r in range(0,nrow):
    for c in range(0,ncol):
        ...
        cent_x = (xoff[c]+xoff[c+1])/2.0
        cent_y = (yoff[r]+yoff[r+1])/2.0
        
        this_cent_pt = [[cent_x,cent_y]]
        #--if rotation is non-zero
        if offset[2] != 0.0:
            this_cent_pt = rotate(this_cent_pt,offset[2])
        
        #--add the offset in after the rotation
        this_cent_pt = add_offset(this_cent_pt,offset)

        wr.poly(parts=[this_cent_pt], shapeType=1)
        wr.record(nrow-r,c+1,delr[c],delc[r],cell_count)
        cell_count += 1
        
wr.save(target='broward_grid_cent_pts')
