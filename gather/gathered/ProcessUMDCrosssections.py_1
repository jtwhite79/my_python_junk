...
head_pointer = long( mf_time_list[-1,3] )
#--zeta surfaces to extract
zetaObj = mfb.MODFLOW_CBB(nlay,nrow,ncol,zeta_file)
zeta_pointer = []
for isurf in xrange(0,nsurf):
    zta_text = '    ZETAPLANE  {0}'.format( isurf+1 )
    zeta_time_list = zetaObj.get_time_list(zta_text)
    zeta_pointer.append(long( zeta_time_list[-1,3] ))
#--get head and zeta data
totim,kstp,kper,h,success = headObj.get_array(head_pointer)
z = []
for isurf in xrange(0,nsurf):
    zt,totim,success = zetaObj.get_array(long(zeta_pointer[isurf]))
    z.append( zt )
...
for idx,xsect in enumerate( xsect_list ):
    ...
    b = np.copy( tv[:,1] )
    #--surface water
    h0 = mfd.cellValueAtPoints(pts[:,0],pts[:,1],pts[:,2],xedge,yedge,h[0,:,:])
    ht = np.copy( h0[:,1] )
    sw = findMaxTwoVectors( ht, b )
    dp = np.ma.masked_where(sw<mean_sl,d)
    sp = np.ma.masked_where(sw<mean_sl,sw)
    bp = np.ma.masked_where(sw<mean_sl,b)
    ax.fill_between(dp,y1=sp,y2=bp,color='#E0FFFF',linewidth=0.)
    #--sea-level
    ...
    for ilay in xrange(0,nlay):
        h0 = mfd.cellValueAtPoints(pts[:,0],pts[:,1],pts[:,2],xedge,yedge,h[ilay,:,:])
        for isurf in xrange(0,nsurf+1):
            if isurf == 0:
                zt = np.copy( z[isurf] )
                t = mfd.cellValueAtPoints(pts[:,0],pts[:,1],pts[:,2],xedge,yedge,botm[ilay,:,:])
                b = mfd.cellValueAtPoints(pts[:,0],pts[:,1],pts[:,2],xedge,yedge,zt[ilay,:,:])
            else:
                zt = np.copy( z[isurf-1] )
                t = mfd.cellValueAtPoints(pts[:,0],pts[:,1],pts[:,2],xedge,yedge,zt[ilay,:,:])
                b = mfd.cellValueAtPoints(pts[:,0],pts[:,1],pts[:,2],xedge,yedge,botm[ilay+1,:,:])
            tm = findMinTwoVectors( t[:,1], h0[:,1] )
            t[:,1] = np.copy( tm )    
            bm = findMinTwoVectors( h0[:,1], b[:,1] )
            b[:,1] = np.copy( bm )    
            #--plot fill_between for zone
            ax.fill_between(t[:,0],y1=t[:,1],y2=b[:,1],color=surf_color[isurf],linewidth=0.)    
        #--geometry data
    ...
    ifigure += 1
    fout = os.path.join( OutputDir, 'UMD_XSECT_{0}.png'.format( xsect_name[idx] ) )
