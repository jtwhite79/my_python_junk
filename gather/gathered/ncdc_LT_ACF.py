...
for id in range(0,len(coopid)):
    ...
    print row
    ax = pylab.subplot(6,2,row)
    this_precip[np.where(this_precip>16.0)] = 0.0
    this_precip_diff = this_precip[1:]-this_precip[:-1]
    this_two_diff = this_precip_diff[1:] - this_precip_diff[:-1]
    ax.acorr(this_precip,maxlags=15,lw=3)
    ...
    ax.text(cmin+2,0.8,'Coopid #: '+str(int(coopid[id])))
    ax.text(cmin+2,0.5,'Significance: '+str(np.round(sig_val,3)))#,horizontalalignment='right')
    ax.set_xlabel('Days')
    ax.set_ylabel('Ac.F.')
    row += 1

    this_wetdry.tofile(str(coopid[id])+'_wetdry.dat',sep=',')
pylab.savefig('ACF.png',format='png')    