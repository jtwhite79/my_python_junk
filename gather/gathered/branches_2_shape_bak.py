...
str_cul_attri = get_structures(file)
...
branch_points,branch_lengths,chain_breaks = set_xsec_breaks_dissolve(names,topo_id,\
...
for i in range(len(branch_points)):
    ...
    up_profiles,up_names = get_profiles(names[i],topo_id[i],break_idx,\
                                        branch_lengths[i],xsec_attri,'start')
    dw_profiles,dw_names = get_profiles(names[i],topo_id[i],break_idx,\
                                        branch_lengths[i],xsec_attri,'end')
    #--save the profiles in the format for SWR
    write_profiles(xsec_dir,up_profiles,up_names)         
    write_profiles(xsec_dir,dw_profiles,dw_names)
    ...
    for j in range(1,len(break_idx)):
        ...
        end_idx = break_idx[j]        
        #--make sure the segment is of minimum lenght tol
