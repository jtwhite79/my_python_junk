...
class mtrct(package):
    ...
    def write_file(self):
        ...
        if (self.isothm in [1, 2, 3, 4, 6]):
            self.parent.write_array( f_rct, self.rhob, self.unit_number[0], True, 13, -ncol, 'Bulk density for Layer',ext_base='rhob')
        if (self.isothm in [5, 6]):
            self.parent.write_array( f_rct, self.prsity2, self.unit_number[0], True, 13, -ncol, 'Immobile porosity for Layer',ext_base='prsity2')
        if (self.igetsc > 0):
            self.parent.write_array( f_rct, self.srconc, self.unit_number[0], True, 13, -ncol, 'Sorbed/immobile starting concentration for Layer',ext_base='srconc')
        if (self.isothm > 0):
            self.parent.write_array( f_rct, self.sp1, self.unit_number[0], True, 13, -ncol, 'First sorption parameter for Layer',ext_base='sp1')
        if (self.isothm > 0):
            self.parent.write_array( f_rct, self.sp2, self.unit_number[0], True, 13, -ncol, 'Second sorption parameter for Layer',ext_base='sp2')
        if (self.ireact > 0):
            self.parent.write_array( f_rct, self.rc1, self.unit_number[0], True, 13, -ncol, 'First order reaction rate for liquid phase for Layer',ext_base='rc1')
        if (self.ireact > 0):
            self.parent.write_array( f_rct, self.rc2, self.unit_number[0], True, 13, -ncol, 'First order reaction rate for sorbed phase for Layer',ext_base='rc2')
        f_rct.close()
