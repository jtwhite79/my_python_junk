...
class mfupw(package):
    'Upstream weigthing package class\n'
    ...
    def write_file(self):
        ...
        # LAYTYP array
        self.parent.write_array(f_upw, self.laytyp, self.unit_number[0], False, 2, -40, 'LAYTYP(): Layer type of layers')
        # LAYAVG array
        self.parent.write_array(f_upw, self.layavg, self.unit_number[0], False, 2, -40, 'LAYAVG(): Layer average of layers')
        # CHANI array
        self.parent.write_array(f_upw, self.chani, self.unit_number[0], False, 2, -40, 'CHANI(): Horizontal anisotropy flag of layers')
        # LAYVKA array
        self.parent.write_array(f_upw, self.layvka, self.unit_number[0], False, 2, -40, 'LAYVKA(): Vertical hydraulic conductivity flag of layers')
        # LAYWET array
        self.parent.write_array(f_upw, self.laywet, self.unit_number[0], False, 2, -40, 'LAYWET(): Wet-Dry flag of layers')
        transient = not self.parent.get_package('DIS').steady.all()
        for i in range(nlay):
        	comment = 'HK() = Horizontal hydraulic conductivity of layer ' + str(i + 1)
        	self.parent.write_array( f_upw, self.hk[:,:,i], self.unit_number[0], True, 13, ncol, comment )
        	if self.chani[i] < 1:
	        	comment = 'HANI() = Ratio of horizontal hydraulic of columns to rows of layer ' + str(i + 1)
	        	self.parent.write_array( f_upw, self.hani[:,:,i], self.unit_number[0], True, 13, ncol, comment )
        	comment = 'VKA() = Vertical hydraulic conductivity of layer ' + str(i + 1)
        	self.parent.write_array( f_upw, self.vka[:,:,i], self.unit_number[0], True, 13, ncol, comment )
        	if transient == True:
        		comment = 'Ss() = Specific storage coefficient of layer ' + str(i + 1)
        		self.parent.write_array( f_upw, self.ss[:,:,i], self.unit_number[0], True, 13, ncol, comment )
        		if self.laytyp[i] !=0:
        			comment = 'Sy() = Specific yield of layer ' + str(i + 1)
        			self.parent.write_array( f_upw, self.sy[:,:,i], self.unit_number[0], True, 13, ncol, comment )
        	if self.parent.get_package('DIS').laycbd[i] > 0:
        		...
        		self.parent.write_array( f_upw, self.vkcb[:,:,i], self.unit_number[0], True, 13, ncol, comment )
        f_upw.close()
